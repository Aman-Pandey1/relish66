...
export default function ProductCard({ product }){
	...
	return (
		<motion.div whileHover={{ y:-6, boxShadow:'0 10px 24px rgba(0,0,0,0.12)' }} transition={{ type:'spring', stiffness:250, damping:20 }} className="group border rounded-lg overflow-hidden bg-white transition relative">
			<div className="absolute top-2 left-2 bg-burnt-600 text-white text-xs rounded-full px-2 py-1 shadow z-10">${finalPrice.toFixed(2)}</div>
			{hasDiscount && (
				<div className="absolute top-2 right-12 bg-burnt-600 text-white text-xs rounded-full px-2 py-1 shadow z-10">-{Math.round(Number(product.discountPercent))}%</div>
			)}
			<button aria-label="Wishlist" className="absolute top-2 right-2 z-10 bg-white/90 hover:bg-white rounded-full px-2 py-1 text-lg shadow" onClick={(e)=>{ e.preventDefault(); toggle(product._id); }}>
				<span className={inWishlist? 'text-burnt-600' : 'text-neutral-600'}>❤</span>
			</button>
			<Link to={`/product/${product.slug}`} className="relative z-0">
				...
				<div className="p-3">
					...
					{hasDiscount && (
						<div className="mt-1">
							<span className="text-burnt-600 font-semibold">${finalPrice.toFixed(2)}</span>
